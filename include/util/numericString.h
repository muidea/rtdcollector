...
#ifndef __NUMBERSTRING_H_1515__
#define __NUMBERSTRING_H_1515__
...
#include <cmath>
#include <cctype>
#if !defined(PLATFORM_NO_LOCALE)
	#include <locale>
#endif


// binary numbers are supported, thus 64 (bits) + 1 (string terminating zero)
#define PLATFORM_MAX_INT_STRING_LEN 65
// value from strtod.cc (double_conversion::kMaxSignificantDecimalDigits)
#define PLATFORM_MAX_FLT_STRING_LEN 780

#define PLATFORM_FLT_INF "inf"
#define PLATFORM_FLT_NAN "nan"
#define PLATFORM_FLT_EXP 'e'
	...
	/// Formatted string has at least [width] total length.
{
	...
	size = ptr - result;
	util_assert_dbg (size <= ptr.span());
	util_assert_dbg ((-1 == width) || (size >= std::size_t(width)));
	*ptr-- = '\0';
...
{
	char res[PLATFORM_MAX_INT_STRING_LEN] = {0};
	std::size_t size = PLATFORM_MAX_INT_STRING_LEN;
...
Util_API void floatToStr(char* buffer,
	int bufferSize,
	float value,
...
Util_API void floatToFixedStr(char* buffer,
	int bufferSize,
	float value,
	int precision);
	/// Converts a float value to string. Converted string must be shorter than bufferSize.
...
Util_API std::string& floatToStr(std::string& str,
	float value,
	int precision = -1,
	int width = 0,
	char thSep = 0,
...
Util_API std::string& floatToFixedStr(std::string& str,
	float value,
	int precision,
	int width = 0,
	char thSep = 0,
...
Util_API void doubleToStr(char* buffer,
	int bufferSize,
	double value,
...
Util_API void doubleToFixedStr(char* buffer,
	int bufferSize,
	double value,
	int precision);
	/// Converts a double value to string. Converted string must be shorter than bufferSize.
...
Util_API std::string& doubleToStr(std::string& str,
	double value,
	int precision = -1,
	int width = 0,
	char thSep = 0,
...
Util_API std::string& doubleToFixedStr(std::string& str,
	double value,
	int precision = -1,
	int width = 0,
	char thSep = 0,
...
Util_API float strToFloat(const char* str);
...
Util_API bool strToFloat(const std::string&, float& result, char decSep = '.', char thSep = ',');
	/// Converts the string of characters into single-precision floating point number.
...
Util_API double strToDouble(const char* str);
	/// Converts the string of characters into double-precision floating point number.


Util_API bool strToDouble(const std::string& str, double& result, char decSep = '.', char thSep = ',');
	/// Converts the string of characters into double-precision floating point number.
	/// The conversion result is assigned to the result parameter.
	/// If decimal separator and/or thousand separator are different from defaults, they should be
	/// supplied to ensure proper conversion.
