...
#define UNIMPLEMENTED util_bugcheck
#include "diy-fp.cc"
...
#include "util/numericString.h"
util_static_assert(PLATFORM_MAX_FLT_STRING_LEN == double_conversion::kMaxSignificantDecimalDigits);
	...
	/// Used only internally.
{
	...
	util_assert_dbg (precision > 0);
	util_assert_dbg (str.length());
...
void floatToStr(char* buffer, int bufferSize, float value, int lowDec, int highDec)
{
	...
	DoubleToStringConverter dc(flags, PLATFORM_FLT_INF, PLATFORM_FLT_NAN, PLATFORM_FLT_EXP, lowDec, highDec, 0, 0);
...
void floatToFixedStr(char* buffer, int bufferSize, float value, int precision)
{
	...
	int flags = DoubleToStringConverter::UNIQUE_ZERO |
		DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
	DoubleToStringConverter dc(flags, PLATFORM_FLT_INF, PLATFORM_FLT_NAN, PLATFORM_FLT_EXP, -std::numeric_limits<float>::digits10, std::numeric_limits<float>::digits10, 0, 0);
...
std::string& floatToStr(std::string& str, float value, int precision, int width, char thSep, char decSep)
{
	...
	char buffer[PLATFORM_MAX_FLT_STRING_LEN];
	floatToStr(buffer, PLATFORM_MAX_FLT_STRING_LEN, value);
	str = buffer;
...
std::string& floatToFixedStr(std::string& str, float value, int precision, int width, char thSep, char decSep)
{
	...
	char buffer[PLATFORM_MAX_FLT_STRING_LEN];
	floatToFixedStr(buffer, PLATFORM_MAX_FLT_STRING_LEN, value, precision);
	str = buffer;
...
void doubleToFixedStr(char* buffer, int bufferSize, double value, int precision)
{
	...
	int flags = DoubleToStringConverter::UNIQUE_ZERO |
		DoubleToStringConverter::EMIT_POSITIVE_EXPONENT_SIGN;
	DoubleToStringConverter dc(flags, PLATFORM_FLT_INF, PLATFORM_FLT_NAN, PLATFORM_FLT_EXP, -std::numeric_limits<double>::digits10, std::numeric_limits<double>::digits10, 0, 0);
	dc.ToFixed(value, precision, &builder);
	builder.Finalize();
}
...
std::string& doubleToStr(std::string& str, double value, int precision, int width, char thSep, char decSep)
{
	...
	char buffer[PLATFORM_MAX_FLT_STRING_LEN];
	doubleToStr(buffer, PLATFORM_MAX_FLT_STRING_LEN, value);
...
std::string& doubleToFixedStr(std::string& str, double value, int precision, int width, char thSep, char decSep)
{
	...
	char buffer[PLATFORM_MAX_FLT_STRING_LEN];
	doubleToFixedStr(buffer, PLATFORM_MAX_FLT_STRING_LEN, value, precision);
...
float strToFloat(const char* str)
{
	...
	StringToDoubleConverter converter(flags, 0.0, Single::NaN(), PLATFORM_FLT_INF, PLATFORM_FLT_NAN);
...
double strToDouble(const char* str)
{
	...
	int flags = StringToDoubleConverter::ALLOW_LEADING_SPACES |
		StringToDoubleConverter::ALLOW_TRAILING_SPACES;
	StringToDoubleConverter converter(flags, 0.0, Double::NaN(), PLATFORM_FLT_INF, PLATFORM_FLT_NAN);
